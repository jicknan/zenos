...
configall = "/etc/X11/xorg.conf"
# The keymap settings file for the current user
...
xorgconf = """# xorg.conf (X.Org X Window System server configuration file)
#
# This file was generated by xkmap, specifically to set the keyboard mapping
#
...
abouttext = _('<i>xkmap</i> is a simple gui front-end for <i>setxkbmap</i>,'
                ...
                ' systems (based on Archlinux) but may work on other'
                ' GNU/Linux systems.\n'
...
either on a global level (for all users) by changing
xorg.conf or just for the current user, but in that case
it depends on how you start X - at some point the script
xkmap-set must be run. One possibility would be to run
it from your .xinitrc file, or maybe your desktop has an
Autorun folder.

In order to set the map globally you must
be running 'xkmap' as root, or else enter the root
password when prompted. xkmap does not (at present) support
input device hot-plugging with evdev, so this must be
disabled in xorg.conf. If no xorg.conf was being used, xkmap
can generate a fresh default one. It can also edit an
existing one, but only if the necessary markers are present
in the keyboard section, e.g.:
...
class Xkbset:
    def __init__(self):
        self.oldxconf = self.readXorgConf()

        if os.path.isfile(configuser):
            f = open(configuser)
    ...
    def readXorgConf(self):
        """Read xorg.conf and extract the keyboard mapping, if possible.
        If not possible (either because it is not straightforard or not
        present, use standard settings as the starting point.
        Return a list of all lines of xorg.conf (either the existing one
        or the default one from xorgconf).
        self.xofile is 'None' if there was no xorg.conf, 'True' if there
        is one with the xkmap markers, else 'False'.
        """
        self.xofile = None
        ...
        if os.path.isfile(configall):
            fh = open(configall)
            xoc = fh.read().splitlines()
            fh.close()

            found = False
            r1 = re.compile(r"\s*" + XKMAPMARK1)
            r2 = re.compile(r"\s*" + XKMAPMARK2)
            rx = re.compile(r'\s*Option\s+\"([a-zA-Z]+)\"\s+\"(.+)\"')
            for line in xoc:
                if found:
                    if r2.match(line):
                        self.xofile = True
                ...
                if rm:
                    ...
                    if k == "XkbModel":
                        ...
                        # test here for 'layout(variant)'
                        rm = re.match(r"(.+)\((.+)\)", v)
                        if rm:
                            self.layout = rm.group(1)
                            self.variant = rm.group(2)
    ...
    def new(self):
        ...
        if (savemode == 'user'):
            ...
            # If there is no xorg.conf, use default (above) and put up
            # message saying that this has been done.
            # If there is one, but it doesn't have the required markers,
            # put up message and change only current map.
            if self.xofile == False:
                popupMessage(_("Previous xorg.conf does not have xkmap"
                        "markers - unable to set global keymap"))
            else:
                if self.xofile == None:
                    popupMessage(_("No previous xorg.conf,"
                            " generating new one"))

                if os.path.isfile(configuser):
                    try:
                        os.remove(configuser)
                    except:
                        pass
                ...
                r1 = re.compile(r"\s*" + XKMAPMARK1)
                r2 = re.compile(r"\s*" + XKMAPMARK2)
                for line in self.oldxconf:
                    if found == 1:
                        f.write(XKMAPMARK1 + '\n')
                        f.write('Option      "XkbModel"      "%s"\n' %
                                self.model)
                        f.write('Option      "XkbLayout"     "%s"\n' %
                                self.layout)
                        if self.variant != "Standard":
                            f.write('Option      "XkbVariant"    "%s"\n' %
                                    self.variant)
                        f.write(XKMAPMARK2 + '\n')
                        found = 2
                        continue
                    ...
                    elif found == 0 and r1.match(line):
                        found = 1
                        continue
